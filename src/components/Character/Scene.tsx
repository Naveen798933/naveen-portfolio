...
import { getOptimalPixelRatio, isLowEndDevice } from "../../utils/performance";

interface LightEngine {
  setPointLight: (screenLight: THREE.Mesh) => void;
  turnOnLights: () => void;
  updateThemeColors: (themeName: string) => void;
}
...
const Scene = () => {
  ...
  const lightEngineRef = useRef<LightEngine | null>(null);

  const [character, setChar] = useState<THREE.Object3D | null>(null);

  useEffect(() => {
    if (lightEngineRef.current) {
      lightEngineRef.current.updateThemeColors(theme);
    }
  }, [theme]);

  useEffect(() => {
    if (canvasDiv.current) {
      const canvasRefCurrent = canvasDiv.current;
      ...
      lightEngineRef.current = light;
      light.updateThemeColors(theme);
